  ...
  return $("li.product article.card")
    .map((index, articleNode) => {
      ...
      article.find(".card-footer .button, .card-bulkOrder-action .button").each((_, node) => {
        ...
        const href = $(node).attr("href") || $(node).closest("form").attr("action") || "";
        if (href) {
          const actionUrl = new URL(href, pageUrl).toString();
          if (label === "Buy Now" && !listingActionUrls.buyNow) {
            listingActionUrls.buyNow = actionUrl;
          }
          if (label === "Add to Cart" && !listingActionUrls.addToCart) {
            listingActionUrls.addToCart = actionUrl;
          }
        }
      });
