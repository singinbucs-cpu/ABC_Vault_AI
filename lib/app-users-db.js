...
async function ensureAppUsersTable() {
  ...
  await getPool().query(`
    CREATE INDEX IF NOT EXISTS app_users_active_email_idx
    ON app_users (email)
    WHERE is_active = TRUE;
  `);

  await getPool().query(`
    CREATE INDEX IF NOT EXISTS app_users_active_forwarding_email_idx
    ON app_users (vault_key_forwarding_email, email)
    WHERE is_active = TRUE AND vault_key_forwarding_email IS NOT NULL;
  `);

  await getPool().query(`
    CREATE INDEX IF NOT EXISTS app_users_active_pushover_email_idx
    ON app_users (email)
    WHERE is_active = TRUE AND pushover_user_key IS NOT NULL AND pushover_user_key <> '';
  `);
